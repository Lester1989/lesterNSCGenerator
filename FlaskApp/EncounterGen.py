...
def HandleSeed(encounter, nscNumber, chars, token, counter=0):
    ...
    if 'seed' in encounter['nscConditions'][nscNumber]:
        ...
        if token in encounter['nscConditions'][nscNumber]['nachname']:
            ...
            seed = GetRandomVorname() + '_' + chars[tokenField[0]][tokenField[1]]
            encounter['nscConditions'][nscNumber]['nachname'] = chars[tokenField[0]][tokenField[1]]
        else:
            seed = GetRandomVorname() + '_' + encounter['nscConditions'][nscNumber]['nachname']
    return seed
        ...
        if token == 'CHAR':
            ...
            while token in encounterText:
                ...
                if encounter['nscConditions'][nscNumber]['art'] in ['Human', 'Kinfolk', 'Garou', 'Ragabash', 'Theurge', 'Philodox', 'Galliard', 'Ahroun']:
                    ...
                    bane = MakeBane(
                        seed=-1 if 'adjectiv' not in encounter['nscConditions'][nscNumber] else random.choice(encounter['nscConditions'][nscNumber]['adjectiv']) + '_' + random.choice(encounter['nscConditions'][nscNumber]['baneType']),
                        powerlevel=0 if 'Powerlevel' not in encounter['nscConditions'][nscNumber] else encounter['nscConditions'][nscNumber]['Powerlevel']
                    )
                    chars[f'{token}{nscNumber}'] = bane
                    encounterText = encounterText.replace(f'({token}{nscNumber})', f'({bane["link"]})')
                    pass
